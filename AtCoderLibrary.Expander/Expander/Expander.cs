...
namespace AtCoder
{
    public static class Expander
    {
        ...
        private static readonly AclFileInfoDetail s_expanderFileInfo
        ...
        internal static readonly AclFileInfoDetail[] s_aclFileInfoDetails = AutoGenerated__SourceInfo.FileInfo.Select(f => new AclFileInfoDetail(f)).Append(s_expanderFileInfo).ToArray();
        ...
        public static void Expand([CallerFilePath] string? inputFilePath = null, string? outputFilePath = null, bool checkLastWriteTime = false, ExpandMethod expandMethod = ExpandMethod.All)
        {
            ...
            var expander = CodeExpander.Create(code, expandMethod, s_aclFileInfoDetails);
            var head = checkLastWriteTime ? $"// Last: {inputFileInfo.LastWriteTimeUtc.Ticks}" : null;

            using var writer = new Writer.FileExpandWriter(expander, outputFilePath, head);
            writer.Expand();
        }
    }
}
